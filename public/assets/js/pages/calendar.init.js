...
function convertD(date){
  console.log(date)
  var tail = date.toString().split('GMT')[1].split(' ')[0]
  var h = tail.substring(0,3)
  var m = tail.substring(3,5)
  var head = date.toISOString().split('.')[0]
  return head+"+00:"+m
}
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        Object.values(data.data).forEach(val => {
          ...
          res.push({
            id: id,
            calendarId: Math.floor(Math.random() * 6) + 1,
            body: val.ghiChu,
            title: 'Lịch hẹn học viên ' + val.ten,
            category: 'time',
            location: 'Trung tâm đào tạo lái xe VMU',
            attendees : ['Admin', val.ten],
            start: convertD(new Date(val.ngayHen)),
            end: convertD(new Date(val.ngayHen).addHour(4)),
          });
        });
        l.clear(), l.createSchedules(res), h();
      }
    });
  }
  function f(e) {
